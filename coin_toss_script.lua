...
local SyncCoins = Events and Events:FindFirstChild("SyncCoins")
...
local S = {
    AutoThrow = false, AutoBuy = false, AutoUpgrade = false, AutoSell = false,
    SelectedCoin = "Basic Coin", TargetMulti = 0.95,
...
local upgradeKeys = {"MoreLuck", "InsaneLuck", "DoubleCash", "DoubleThrow"}
...
local function getMultiplier()
    ...
    if cm then return cm.Size.Y.Scale end
    return nil
end

local function getSelectedCoin()
    local pg = LP:FindFirstChild("PlayerGui")
    local coinMain = pg and pg:FindFirstChild("UiFolder") and pg.UiFolder:FindFirstChild("Main")
        and pg.UiFolder.Main:FindFirstChild("HUD") and pg.UiFolder.Main.HUD:FindFirstChild("Coin")
        and pg.UiFolder.Main.HUD.Coin:FindFirstChild("Main")
    local cn = coinMain and coinMain:FindFirstChild("CoinName")
    return cn and cn.Text or "?"
end

local function getCoinsTable()
    local list = {}
    local coinsFolder = RS:FindFirstChild("Assets") and RS.Assets:FindFirstChild("Assets") and RS.Assets.Assets:FindFirstChild("Coins")
    if coinsFolder then
        for _, c in ipairs(coinsFolder:GetChildren()) do table.insert(list, c.Name) end
    end
    if #list == 0 then table.insert(list, "Basic Coin") end
    return list
end
...
local function doThrow()
    ...
    if multi >= S.TargetMulti then
        ...
        local ok, err = pcall(function() CoinThrow:FireServer(S.SelectedCoin, targetPos) end)
        if ok then print("[投币] " .. S.SelectedCoin .. " @mult=" .. string.format("%.2f", multi)) end
        wait(0.5)
    end
end
...
local function doBuyCoins()
    ...
    local pkg = require(RS.Assets.Modules.ProgressionModule)
    for _, coinObj in ipairs(coinsFolder:GetChildren()) do
        local coinName = coinObj.Name
        local cost = pkg.Coins[coinName] and pkg.Coins[coinName].Cost
        if cost and cash.Value >= cost then
            local ok, err = pcall(function() BuyCoin:FireServer(coinName) end)
            if ok then print("[购买] " .. coinName .. " $" .. cost); wait(0.2) end
        end
    end
end

local function doUpgrade()
    if not S.AutoUpgrade or not RequestUpgrade then return end
    for _, key in ipairs(upgradeKeys) do
        local ok, err = pcall(function() RequestUpgrade:FireServer(key) end)
        if ok then print("[升级] " .. key); wait(0.2) end
    end
end
...
local function sP()
    ...
    PS = {}; wait(0.3)
    local sg = Instance.new("ScreenGui"); sg.Name = "CP"; sg.ResetOnSpawn = false; sg.DisplayOrder = 999999
    ...
    for i = 1, 50 do
        ...
        d.BackgroundColor3 = S.ParticleColor; d.BackgroundTransparency = 0.3+math.random()*0.5
...
local function mW()
    WN = WI:CreateWindow({
        Title="扔硬币 v2.0", Author="b站英吉利超入_", Icon="solar:wallet-bold",
        ...
        OnClose=function() xP(); S.AutoThrow=false; S.AutoBuy=false; S.AutoUpgrade=false; S.AutoSell=false; S.Speed=false; S.Fly=false; doSpeed=false; doFly=false
    ...
    local coinOptions = getCoinsTable()
    CT.CoinSelect = t1:Dropdown({Flag="CoinSelect", Title="投币类型", Values=coinOptions, Value="Basic Coin", Callback=function(v) S.SelectedCoin=v end})
    ...
    CT.AutoUpgrade = t1:Toggle({Flag="AutoUpgrade", Title="自动升级(运气+倍率)", Value=false, Callback=function(v) S.AutoUpgrade=v end})
    ...
    t2:Keybind({Flag="ToggleKey", Title="窗口开关", Value="RightShift", Callback=function(v) KB.Toggle=v; pcall(function() WN:SetToggleKey(v) end) end})
    ...
    t6:Paragraph({Title="扔硬币 v2.0"}); t6:Divider()
    t6:Paragraph({Title="作者", Desc="b站英吉利超入_"})
    t6:Paragraph({Title="说明", Desc="自动投币/购买/升级/出售 + 飞行加速"})
...
UIS.InputBegan:Connect(function(input, gpe)
    ...
    if input.UserInputType == Enum.UserInputType.Keyboard then
        ...
        if kn == KB.Toggle and WN then
            pcall(function()
                if WN.FolderOpen then WN:Close() else WN:Open() end
            end)
        end
    end
end)
...
spawn(function()
    ...
    while true do
        ...
        if S.AutoUpgrade then pcall(doUpgrade) wait(0.3) end
